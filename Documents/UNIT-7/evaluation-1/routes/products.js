...
productRouter.put("/products/:productId", (req, res) => {
  const found = products.products.some(
  ...
  if (found) {
    ...
    res
      .status(400)
      .json({ msg: `No product with id of ${req.params.productId}` });
  }
});
...
productRouter.delete("/products/:productId", (req, res) => {
  ...
  if (found) {
    res.send({
      msg: "product deleted",
      product: products.products.filter(
        (product) => product.id !== parseInt(req.params.productId)
      ),
    });
  } else {
    res
      .status(400)
      .json({ msg: `No product with the id of ${req.params.productId} ` });
  }
});
